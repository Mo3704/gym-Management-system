...
<script>
    ...
    function validateInstallmentAmounts() {
        ...
        var totalPrice = parseFloat(document.getElementById("price").value.substr(1)); // Extract total price without '$'
        var installmentAmountInputs = document.querySelectorAll('input[name="installment-amount[]"]');
        var totalPaidAmount = 0;

        for (var i = 0; i < installmentAmountInputs.length; i++) {
            var amount = parseFloat(installmentAmountInputs[i].value);
            if (!isNaN(amount)) {
...
<?php
    ...
    if (isset($_SESSION['selected_plan'])) {
        ...
        // Calculate expiration date based on plan validity
        $enroll_date = date('Y-m-d'); // Default to current date
        $validity_days = htmlspecialchars($selected_plan['validity']) ?? 0;
     }

    // Fetch the last member ID from the database
    $last_member_id_query = "SELECT Id FROM clients ORDER BY Id DESC LIMIT 1";
    $last_member_id_result = mysqli_query($connection, $last_member_id_query);

    // Initialize member ID variable
    $next_member_id = 1;

    // If there are existing member IDs in the database, increment the last ID by one
    if ($last_member_id_result && mysqli_num_rows($last_member_id_result) > 0) {
        $last_member_id_row = mysqli_fetch_assoc($last_member_id_result);
        $next_member_id = intval($last_member_id_row['Id']) + 1;
    }

    // Close the previous query
    mysqli_free_result($last_member_id_result);
?>


<h1>Add New Payment</h1>
<form class="gym-form" action="payment.php" method="POST" onsubmit="return  confirmFullPayment() && validateInstallmentAmounts();">
   <div class="form-group">
        <label for="member-id">Member ID:</label>
        <input type="text" id="member-id" name="member-id" value="<?php echo $next_member_id; ?>" readonly required>
     </div>
    <div class="form-group">
